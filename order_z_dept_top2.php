...
<table border="0" cellpadding="0" cellspacing="2">
    <tr>
        ...
        */
        $sql = "
    SELECT int_id, chr_name, int_sort, status ,start_time ,end_time
	...
	ORDER BY int_sort;";
        ...
        if ($starttime){
            //柯打時間不在時間段內
            if ($starttime > $ordertime) $catExpired = true;
        }

        if ($endtime){
            //柯打時間不在時間段內
            if ($endtime < $ordertime) $catExpired = true;
        }
        ...
        IF ($count <= 6) {
            ...
            <td width="106" height="38" align="center" background="<?= $bgimage; ?>"><a
                        href="order_z_dept_middle.php?catid=<?= rawurlencode($record['int_id']); ?>&bl_select=<?= $bl_select ?>"
                        target="middleFrame"
                        onClick="parent.mainFrame.location.href='order_z_bottom.php'"><?= $record['chr_name']; ?></a>
            </td>
            <?php
        } ELSE {
        ?>
    ...
    <tr>
        <td width="106" height="38" align="center" background="<?= $bgimage; ?>"><a
                    href="order_z_dept_middle.php?catid=<?= rawurlencode($record['int_id']); ?>&bl_select=<?= $bl_select ?>"
                    target="middleFrame"
                    onClick="parent.mainFrame.location.href='order_z_bottom.php'"><?= $record['chr_name']; ?></a></td>
        <?php
        $count = 1;
        }
        $count += 1;
