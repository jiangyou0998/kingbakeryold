                    ...
                    <tr>
                        <td>
                            <select name="menu1" onChange="MM_jumpMenu('self',this,1)">
                                <option value="?dept=0&type=0"<?php if ($_REQUEST[type] == "0") {
                                    echo "selected";
                                } ?>>請選擇帳戶類型
                                </option>
                                <option value="?dept=0&type=2" <?php if ($_REQUEST[type] == 2) echo "selected" ?>>分店
                                </option>
                                <option value="?dept=0&type=1" <?php if ($_REQUEST[type] == 1) echo "selected" ?>>
                                    一般使用者
                                </option>
                                <option value="?dept=0&type=3" <?php if ($_REQUEST[type] == 3) echo "selected" ?>>
                                    系統管理員
                                </option>
                            </select>
                            <?php if ($_REQUEST['type'] <> 2 && $_REQUEST['type'] <> 0) { ?>
                                <select name="menu2" onChange="MM_jumpMenu('self',this,1)">
                                    <option value="?type=<?= $_REQUEST[type] ?>&dept=0"<?php if ($_REQUEST[dept] == "0") {
                                        echo "selected";
                                    } ?>>選擇部門
                                    </option>
                                    <?php
                                    ...
                                    WHILE ($record = mysqli_fetch_array($result)) { ?>
                                        <option value="?type=<?= $_REQUEST[type] ?>&dept=<?= $record[int_id] ?>"<?php if ($_REQUEST[dept] == $record[int_id]) {
                                            echo "selected";
                                        } ?>><?= $record[txt_dept] ?></option>
                                    <?php } ?>
                                </select>
                            <?php } ?>
                            <input name="Add" type="button" id="Add" value="新增使用者"
                                   onClick="document.location.href='CMS_user2.php?action=new';">
                        </td>
...
<form name="update" action="CMS_user.php?dept=<?= $_REQUEST[dept] ?>&type=<?= $_REQUEST[type] ?>&action=update"
      method="post">
    <table width="90%" border="0" cellspacing="1" cellpadding="6">
        <tr>
        ...
        WHILE ($record = mysqli_fetch_array($result)) {
            ...
            <tr>
                ...
                <td width="40" align="center" bgcolor="<?php echo $bg; ?>"><a
                            href="CMS_user2.php?action=edit&id=<?php echo $record[int_id]; ?>">更新</a></td>
                <td width="40" align="center" bgcolor="<?php echo $bg; ?>"><a
                            href="CMS_user2.php?action=delete&id=<?php echo $record[int_id]; ?>">刪除</a></td>
            </tr>
            <?php
            $count += 1;
        }
