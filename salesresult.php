...
<div style="margin:1%;">
    ...
    <form action="?action=search" method="POST" align="center" style="width:995">
        查閱數據日期:
        由 <input name="date1" onclick="WdatePicker();" size="10" style="text-align:center;"
                 value="<?= $date1 ? $date1 : date("Y-m-d", gettimeofday('sec')) ?>"/>
        至 <input name="date2" onclick="WdatePicker();" size="10" style="text-align:center;"
                 value="<?= $date2 ? $date2 : date("Y-m-d", gettimeofday('sec')) ?>"/>
        <input type="submit" value="查閱">
    </form>

    <br>
    數據日期：由 <?= $date1 ? $date1 : date("Y-m-d", gettimeofday('sec')) ?>
    至 <?= $date2 ? $date2 : date("Y-m-d", gettimeofday('sec')) ?>
    <br/>
