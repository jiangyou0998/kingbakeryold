...
require($DOCUMENT_ROOT . "connect.inc");
 if($_REQUEST[action] =="AddLibrary"){
		...
		$select_shop =0;$cent =0;$logs =0;$dept=0;
		if($select_state==1){
			$select_shop = $_REQUEST[select_shop];	//指定分店可見
		}else{
			if($_REQUEST[cent]){	$cent=1;	}	//所有分店可見
			if($_REQUEST[logs]){	$logs=1;	}	//所有後勤可見
			if($_REQUEST[dept]){	$dept=1;	}
		}
			
			
			foreach($_FILES['txt_url']['type'] as $key => $value){
				if($value !== "application/pdf"){
					echo "<script>alert('顯示文檔必須為PDF');document.location.href='CMS_library.php';</script>";die;
				}	
			}
