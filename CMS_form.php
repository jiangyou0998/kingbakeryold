...
<?php 
  ...
  echo $record_user[0];?></td>
	...
	<td align="left" bgcolor="<?php  echo $bg;?>"><?php  if($record['first_path']==''){echo "---";}else{echo '<a href="forms/'.$record['first_path'].'" target="_blank">'.$record['first_path'].'</a>';}?></td>
	
	<td align="left" bgcolor="<?php  echo $bg;?>"><input name="<?=$record[0]?>" type="checkbox" value="1"<?=$record[12] ? "checked" : ""?> class="cbUpdate"></td>
    <td align="left" bgcolor="<?php  echo $bg;?>">
		<input name="del" type="button" id="del" value="刪除" onClick="return confirmation(14,<?php echo $record[0];?>,'<?php echo $record[1];?>');">
	</td>
    <td align="left" bgcolor="<?php  echo $bg;?>">
		<?php IF ($record[9] == 1) {?>
