...
if ($_REQUEST[action] == 'UPDATE') {
    ...
    $logs = 0;
    $dept = 0;
    if ($_REQUEST['cent' . $pmd_id]) {
        $cent = 1;
    }//分店
    if ($_REQUEST['logs' . $pmd_id]) {
        $logs = 1;
    }//後勤
    if ($_REQUEST['dept' . $pmd_id]) {
        $dept = 1;
    }//後勤
    if ($fileName == "" && $oriname == "") {
...
//	document.getElementById("url2"+op).style.display="none";
        ...
        function Change_main(op, id) {
            $.post("CMS_library_do.php", {
                'action': "list_do",
                'pram': op.substring(0, op.indexOf('.')),
                'id': id
            }, function (o) {
        ...
        <div align="left" style="width:1164px">
            ...
            <select name="type" id="type" onChange="changeClik(this)" class="cssInput">
                ...
                while ($rec = mysqli_fetch_array($res)) {
                    ?>
                    <option value="<?= $rec[int_id] ?>"
                            <?php if ($_REQUEST[type_id] == $rec[int_id]){ ?>selected="selected"<?php } ?>><?= $rec[txt_name] ?></option>
                    <?php
                }
                ?>
            </select>
        ...
        <form enctype="multipart/form-data" name="form1" method="post" action="">
            <table width="1164" height="65" border="1" cellpadding="0" cellspacing="0" style="text-align:center">
                <tr>
                    ...
                    <?php }else{ ?>
                    <form enctype="multipart/form-data" name="form1" method="post" action="">
                        <table width="980" height="65" border="1" cellpadding="0" cellspacing="0"
                                ...
                                <tr onMouseOver="this.style.background='#AFD1DD'" onMouseOut="this.style.background=''">
                                    <?php if ($_REQUEST[action] != 'VIEW_ONLY') { ?>
                                        <td width="100" height="38">
                                            <div id="m_name1<?= $rec[pmd_id] ?>"
                                                 style="display:block"><?= $rec[m_name] ?>&nbsp;
                                            </div>
                                            <div id="m_name2<?= $rec[pmd_id] ?>" style="display:none">
                                                <select name="select_main<?= $rec[pmd_id] ?>"
                                                        id="select_main<?= $rec[pmd_id] ?>"
                                                        onChange="Change_main(this.value,<?= $rec[pmd_id] ?>)"
                                                        style='width:90px;'>
                                                    <?php
                                    ...
                                    <td width="180" height="38">
                                        <div id="g_name1<?= $rec[pmd_id] ?>" style="display:block"><?= $rec[g_name] ?>
                                            &nbsp;
                                        </div>
                                        <div id="g_name2<?= $rec[pmd_id] ?>" style="display:none">
                                            ...
                                            if ($rec[mn_id] != "") {
                                                ?>
                                                <select name="select_group<?= $rec[pmd_id] ?>"
                                                        id="select_group<?= $rec[pmd_id] ?>" style='width:100px;'>
                                                    <?php
                                    ...
                                    <td width="85">
                                        <div id="author<?= $rec[pmd_id] ?>" style="display:block"><?= $rec[author] ?>
                                            &nbsp;
                                        </div>
                                        <!--
                                    ...
                                    <td width="214">
                                        <div id="name1<?= $rec[pmd_id] ?>" style="display:block"><?= $rec[p_name] ?>
                                            &nbsp;
                                        </div>
                                        <div id="name2<?= $rec[pmd_id] ?>" style="display:none">
                                            <input type="text" id="txt_name<?= $rec[pmd_id] ?>"
                                                   name="txt_name<?= $rec[pmd_id] ?>" size="20"
                                                   value="<?= $rec[p_name] ?>"></div>
                                    </td>
                                    <td width="231">
                                        <div id="path1<?= $rec[pmd_id] ?>" style="display:block"><a target="_new"
                                                                                                    href="<?= $rec[p_path] ?>"><?= $rec[p_path] ?></a>&nbsp;
                                        </div>
                                        <div id="path2<?= $rec[pmd_id] ?>" style="display:none">
                                            <input type="file" size="18" id="txt_path<?= $rec[pmd_id] ?>"
                                                   name="txt_path<?= $rec[pmd_id] ?>" value="<?= $rec[p_path] ?>"></div>
                                    </td>
                                    <td width="445">
                                        <div id="oriname1<?= $rec[pmd_id] ?>"
                                             style="display:block"><?= $rec[p_orifile] ?>&nbsp;
                                        </div>
                                        <div id="oriname2<?= $rec[pmd_id] ?>" style="display:none">
                                            <input type="text" id="txt_oriname<?= $rec[pmd_id] ?>"
                                                   name="txt_oriname<?= $rec[pmd_id] ?>" size="20"
                                                   value="<?= $rec[p_orifile] ?>"></div>
                                    </td>
                                    <!--
	<td width="231">
...
-->
                                    <td width="100">
                                        ...
                                        <div id="shop2<?= $rec[pmd_id] ?>" style="display:none">
                                            <select name="select_shop<?= $rec[pmd_id] ?>"
                                                    id="select_shop<?= $rec[pmd_id] ?>" style='width:100px;'>
                                                <option value="0" <?php if ($rec_shop[int_id] == $rec[shop]) {
                                                ...
                                                while ($rec_shop = mysqli_fetch_array($res_shop)) {
                                                    ?>
                                                    <option value="<?php echo $rec_shop["int_id"] ?>" <?php if ($rec_shop[int_id] == $rec[shop]) {
                                                        echo "selected='selected'";
                                                    } ?>> <?php echo $rec_shop["txt_name"] ?> </option>
                                                    <?php
                                                }
                                                ?>
                                            </select>
                                    ...
                                    <?php if ($_REQUEST[action] != 'VIEW_ONLY') { ?>
                                        <td width="78" align="center" valign="middle">
                                            <div id="cent1<?= $rec[pmd_id] ?>"
                                                 style="display:block"><?php if ($rec[cent] == "1") echo "√"; ?>&nbsp;
                                            </div>
                                            <div id="cent2<?= $rec[pmd_id] ?>" style="display:none">
                                                <input name="cent<?= $rec[pmd_id] ?>" type="checkbox"
                                                       id="cent<?= $rec[pmd_id] ?>" <?php if ($rec[cent] == 1) echo "checked"; ?>>
                                            </div>
                                        </td>

                                        <td width="83" align="center" valign="middle">
                                            <div id="logs1<?= $rec[pmd_id] ?>"
                                                 style="display:block"><?php if ($rec[logs] == "1") echo "√"; ?>&nbsp;
                                            </div>
                                            <div id="logs2<?= $rec[pmd_id] ?>" style="display:none">
                                                <input name="logs<?= $rec[pmd_id] ?>" type="checkbox"
                                                       id="logs<?= $rec[pmd_id] ?>" <?php if ($rec[logs] == 1) echo "checked"; ?>>
                                            </div>
                                        </td>
                                        <td width="83" align="center" valign="middle">
                                            <div id="dept1<?= $rec[pmd_id] ?>"
                                                 style="display:block"><?php if ($rec[depts] == "1") echo "√"; ?>&nbsp;
                                            </div>
                                            <div id="dept2<?= $rec[pmd_id] ?>" style="display:none">
                                                <input name="dept<?= $rec[pmd_id] ?>" type="checkbox"
                                                       id="dept<?= $rec[pmd_id] ?>"
                                                       onClick="select_check(<?= $rec[pmd_id] ?>)" <?php if ($rec[depts] == 1) echo "checked"; ?>>
                                            </div>
                                        </td>
                                        <td width="42">
                                            <input type="button" id="but_save<?= $rec[pmd_id] ?>" style=" display:none"
                                                   value="保存"
                                                   onClick="save_lib(<?= $rec[pmd_id] ?>,<?= $rec[status] ?>)">
                                            <input type="button" id="but_update<?= $rec[pmd_id] ?>" value="修改"
                                                   onClick="update_lib(<?= $rec[pmd_id] ?>,<?= $rec[status] ?>)">
                                        </td>
                                        <td width="48">
                                            <input type="button" id="but_res<?= $rec[pmd_id] ?>" style=" display:none"
                                                   value="取消" onClick="res_lib(<?= $rec[pmd_id] ?>)">
                                            <input type="button" id="but_del<?= $rec[pmd_id] ?>" value="刪除"
                                                   onClick="delete_lib(<?= $rec[pmd_id] ?>)">
                                        </td>
                                    <?php } ?>
                                </tr>
                                <?php
