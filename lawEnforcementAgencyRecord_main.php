...
//echo $sql_insert;
	  mysqli_query($con, 'set name big5');
		...
		if($insert_Result > 0) {
			...
			?><script language="javascript">alert("成功新增巡查事項!"); window.location.href = "lawEnforcementAgencyRecord_main.php"</script><?
			//sendEmail();
			//sendEmail2();
		}
...
//						sliderAccessArgs:{touchonly:false}
		...
		function show(obj,id) {
			...
			wBox=$("#extraInfo"+obj).wBox({requestType: "iframe",target:"lawEnforcementAgencyRecord_additional_utf_8.php?id="+id,title:"跟進資料", width:200,height:400});
			wBox.showBox();
		}
...
<div id="popupPanel" style="display: none;" >
	<div class="popup" name="__popup" onMouseOut="exitPanel(event)">
		<table cellspacing="0" width="222px" height="120px" cellpadding="0" border="0" background="images/order_bg.jpg" style="background-repeat:no-repeat">
			<tr>					
				<td class="description" width="230px" style="padding: 5px; padding-left:8px;padding-top:8px;vertical-align: top;">
...
<form enctype="multipart/form-data" name="form_CaseInsert" id="form" action="lawEnforcementAgencyRecord_main.php" method="post" onSubmit="return confInsertSubmit();">
...
<table border="1" cellspacing="0" cellpadding="1" width="994px">
  ...
  <tr>
    <td align="center"><? if($_SESSION['dept']<>20){echo date('Y-m-d',$timestamp)."<br>";}else{?>
    ...
    <td>
      <table width="100%" border="0" cellpadding="3" cellspacing="0">
        ...
        <tr>
          <td align="right">
            <select name="risk_readLicense" id="risk_readLicense" style="width:105px" onChange="checkRisk()">
    ...
    <td>
      <table width="100%" border="0" cellpadding="1" cellspacing="0">
        <tr>
          ...
          <td><input type="button" id="clear1" name="clear1" onClick="clearFileInputField('1');" value="清除" /></td>
        </tr>
        <tr>
          <td align="right">
            <div id="div_file2">
              <input id="file2" name="uploadfile[]" type="file" onChange="upFile('2');" size="1" style="position:absolute;filter:alpha(opacity=0);  -moz-opacity:0;         /*火狐*/opacity:0;  width:105px;"  />
              <input type="button" id="btnFile2" name="btnFile2"  style="width:116px" value="相片1" />
            </div>
          </td>
          <td><input type="button" id="clear2" name="clear2"  onClick="clearFileInputField('2');" value="清除" /></td>
        </tr>
        <tr>
          <td align="right">
            <div id="div_file3">
              <input id="file3" name="uploadfile[]" type="file" onChange="upFile('3');" size="1" style="position:absolute;filter:alpha(opacity=0);  -moz-opacity:0;         /*火狐*/opacity:0;  width:105px;"  />
              <input type="button" id="btnFile3" name="btnFile3"  style="width:116px" value="相片2" />
            </div>
          </td>
          <td><input type="button" id="clear3" name="clear3"  onClick="clearFileInputField('3');" value="清除" /></td>
        </tr>
        <tr>
          <td align="right">
            <div id="div_file4">
              <input id="file4" name="uploadfile[]" type="file" onChange="upFile('4');" size="1" style="position:absolute;filter:alpha(opacity=0);  -moz-opacity:0;         /*火狐*/opacity:0;  width:105px;"  />
              <input type="button" id="btnFile4" name="btnFile4"  style="width:116px" value="相片3" />
            </div>
          </td>
          <td><input type="button" id="clear4" name="clear4"  onClick="clearFileInputField('4');" value="清除" /></td>
        </tr>
      </table>
    </td>
    <td align="center">
...
<table width="994px" border="0" cellspacing="0" cellpadding="0">
	<tr><td align="center">
		<h2>最近<?=$show_Entry_Months;?>個月內收到之巡查意見</h2>
  			<label <?if($_SESSION['chr_sap'] <> '' && $_SESSION['dept'] == 2) { ?>style="display:none"<?}?>>分店</label>
  			<select id="branchSel" name="branchSel" onChange="rstSel()" <?if($_SESSION['chr_sap'] <> '' && $_SESSION['dept'] == 2) { ?>style="display:none"<?}?>>
...
<form name="form_CaseUpdate" id="form" action="lawEnforcementAgencyRecord_main.php" method="post"> <!--  onSubmit="return confUpdateSubmit();" -->
...
<?
	...
	*/
	...
	if (mysqli_num_rows($result_Notcomplete) == 0) {echo "<table width=\"994px\"><tr><td align=\"center\"><font size=5 color=red>沒有紀錄 !!</font></td></tr>";} else {
?>
...
?>
  <tr>
    <td align="center" bgcolor="<?=$bgcolor;?>"><b><?echo ($risklvl >= 3?"<font color=\"red\">":"").$recordCount.($risklvl >= 3?"</font>":"");?></b></td>
    <td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").$record_Notcomplete[chr_enforce_case_num].($risklvl >= 3?"</font>":"");?></td>
    <td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").$record_Notcomplete[datetime_input_date].'('.$record_Notcomplete[chr_enforce_time].')'.($risklvl >= 3?"</font>":"");?></td>
    ...
    <td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").$record_Notcomplete[chr_enforce_dept].($risklvl >= 3?"</font>":"");?></td>
    <td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").$record_Notcomplete[chr_shopStaff].($risklvl >= 3?"</font>":"");?></td>
    ...
    <td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").str_replace("無","---",$record_Notcomplete[chr_enforce_risk_photo]).($risklvl >= 3?"</font>":"");?></td>
    <td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").str_replace("無","---",$record_Notcomplete[chr_enforce_risk_measure]).($risklvl >= 3?"</font>":"");?></td>
    <td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").str_replace("無","---",$record_Notcomplete[chr_enforce_risk_readLicense]).($risklvl >= 3?"</font>":"");?></td>
...
//							=$record_Notcomplete[chr_old_file_name_4]."</a></td>";}
	...
	<td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").$record_Notcomplete[bool_counter].($risklvl >= 3?"</font>":"");?></td>
	<td align="center" bgcolor="<?=$bgcolor;?>"><?echo ($risklvl >= 3?"<font color=\"red\">":"").$record_Notcomplete[bool_end].($risklvl >= 3?"</font>":"");?></td>    
	
	<td align="center" bgcolor="<?=$bgcolor;?>">
		<input type="button" name="extraInfo<?=$record_Notcomplete[int_id];?>"  onClick="show('<?=$record_Notcomplete[int_id];?>','<?=$record_Notcomplete[int_id];?>')" id="extraInfo<?=$record_Notcomplete[int_id];?>" value="資料"/>
	</td>    
