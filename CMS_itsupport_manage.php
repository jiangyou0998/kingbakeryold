    ...
    if ($submit == "del") {
        ...
        if ($del_Result > 0) {
            ...
            <script language="javascript">alert('刪除紀錄成功!');
                document.location.href = "CMS_itsupport_manage.php"</script><?php
        }
    } else if ($submit == "add") {
        ...
        function show(type, id) {
            switch (type) {
                case 1:
                    wBox = $("#look_").wBox({
                        requestType: "iframe",
                        target: "Itsupport_help.php?id=" + id + "&type=" + type,
                        title: "求助事宜編輯",
                        width: 200,
                        height: 400
                    });
...
<div style=" width:994px; text-align: right;">
    <form name="form_SendUpdate" id="form" action="CMS_itsupport_manage.php?submit=add" method="post"
    ...
    <table width="994px" border="1" style="border-collapse:collapse;" borderColor="black" cellspacing="0"
        ...
        while ($record_Notcomplete = mysqli_fetch_array($result_Notcomplete)) {
            ...
            while ($record_project = mysqli_fetch_array($result_project)) { ?>
                <tr bgcolor="#BEBEBE" class="detail item-<?= $record_Notcomplete[int_id] ?>"
                    ...
                    <td align="center"><a href="javascript:show(1,<?= $record_project[int_id] ?>)">編輯</a> | <a
                                href="?submit=del&id=<?= $record_project[int_id] ?>&type=1"
