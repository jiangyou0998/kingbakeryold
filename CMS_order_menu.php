...
<head>
	...
	<script type="text/JavaScript">
		<!--
		function MM_jumpGroup(targ, selObj, restore) { //v3.0
			var cat = $('#cat').val();
			var url;
			url = "?cat=" + cat;
			url += "&group=0";
		...
		<tr>
			<td rowspan="3">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td>
							<select name="cat" id="cat" onChange="MM_jumpGroup()">
							...
							<select name="group" id="group" onChange="MM_jumpMenu()">
								<option value="0" <?php if ($_REQUEST[group] == "0") {
									echo "selected";
								} ?>>請選擇細類
								</option>
								<?php
							...
							<input name="Add" type="button" id="Add" value="新增貨品"
								   onClick="document.location.href='CMS_order_menu2.php?action=new';">
						</td>
...
<form name="update" action="CMS_order_menu.php?cat=<?= $_REQUEST[cat] ?>&group=<?= $_GET[group] ?>&action=update"
	  method="post">
	<table width="90%" border="0" cellspacing="1" cellpadding="6">
		<tr>
	...
	FROM tbl_order_z_menu T0
		...
		WHILE ($record = mysqli_fetch_array($result)) {
			...
			<tr>
				...
				<td width="40" align="center" bgcolor="<?php echo $bg; ?>"><input name="<?php echo $record[int_id]; ?>"
																				  type="text"
				...
				switch ($record[status]) {
					case 1 :
						$status = '<span class="style2">現貨</span>';
						break;
					case 2 :
						$status = '<span class="style3">暫停</span>';
						break;
					case 3 :
						$status = '<span class="style4">新貨</span>';
						break;
					case 5 :
						$status = '<span class="style5">季節貨</span>';
						break;
				}
				?>
