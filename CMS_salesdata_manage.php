...
<div align="center" style="width:994px">
	...
	<table width="600px" border="1" style="border-collapse:collapse;" borderColor="black" cellspacing="0" cellpadding="4px" align="center">
		<tr bgcolor="CCFFFF">
	...
	<table width="80%" border="1" style="border-collapse:collapse;" borderColor="black" cellspacing="0" cellpadding="4px">
		<tr bgcolor="CCFFFF">
			<td align="center" width="30%"><b>名稱</b></td>
			<td align="center" width="10%"><b>計算</b></td>
			<td align="center" width="30%"><b>狀態</b></td>
		...
		<tr style="background-color:<?=$record[status]==1?'#ADFFAD':'#FFFFAD'?>">
			<td><input type="text"  name="data[<?=$record[int_id]?>][name]" id="txt_name" value="<?=$record[chr_name]?>" /></td>
			<td align="center">
			...
			<td align="center">
				使用中 <input type="radio" name="data[<?=$record[int_id]?>][status]" value="1" <?=$record[status]==1?'checked':''?>/>　
				停用   <input type="radio" name="data[<?=$record[int_id]?>][status]" value="2" <?=$record[status]==2?'checked':''?>/>
			</td>
			<td align="center"><input type="text"  name="data[<?=$record[int_id]?>][sort]" id="txt_sort" value="<?=$record[int_sort]?>" size="5"/></td>
			<td align="center">
				<button type="button" onclick="deleteform.id.value='<?=$record[int_id]?>'; deleteform.submit();" style="background-color:#FFADAD;">刪除</button>
			</td>
		</tr>
		<?php } ?>
	</table>
