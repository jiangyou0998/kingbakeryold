...
<head>
    ...
    <script language="javascript">
        ...
        function addlib(aa) {
            if (bool) {
                document.getElementById("tblMess").style.display = "block";
                bool = false;
            } else {
                document.getElementById("tblMess").style.display = "none";
                bool = true;
            }
        }
...
<div align="center">
    ...
    if (!$_REQUEST[linkState]) {
        ?>
        <div align="right">
            <form enctype="multipart/form-data" onSubmit="return add_lib()" action="?action=main" method="POST"
                  name="form1" id="form1">
                <table height="37">
                    <tr>
                    ...
                    <tr>
                        <td width="431" height="38">
                            ...
                            <div id="name2<?= $rec[int_id] ?>" style="display:none">
                                <input type="text" id="txt_name<?= $rec[int_id] ?>" name="txt_name<?= $rec[int_id] ?>"
                                       size="16" value="<?= $rec[txt_name] ?>">
                            </div>
                        </td>
                        <td width="80" height="38">
                            ...
                            <div id="sort2<?= $rec[int_id] ?>" style="display:none">
                                <input type="text" id="int_sort<?= $rec[int_id] ?>" name="int_sort<?= $rec[int_id] ?>"
                                       size="6" value="<?= $rec[int_sort] ?>">
                            </div>
                        </td>

                        <td width="42">
                            <input type="button" id="but_update<?= $rec[int_id] ?>" value="修改"
                                   onClick="update_lib(<?= $rec[int_id] ?>)">
                            <input type="button" id="but_save<?= $rec[int_id] ?>" style=" display:none" value="保存"
                                   onClick="save_lib(<?= $rec[int_id] ?>,1,0)">
                        </td>
                        <td width="43">
                            <input type="button" id="but_del<?= $rec[int_id] ?>" value="刪除"
                                   onClick="delete_lib(<?= $rec[int_id] ?>,1,0)">
                            <input type="button" id="but_res<?= $rec[int_id] ?>" style=" display:none" value="取消"
                                   onClick="res_lib(<?= $rec[int_id] ?>)">
                        </td>
                    </tr>
                    <?php
                }
        ...
        ?>
        <div align="right">
            <form enctype="multipart/form-data" onSubmit="return add_lib()"
                  action="?action=group&main=<?= $_REQUEST[int_main] ?>" method="POST" name="form1" id="form1">
                <table height="37">
                    <tr>
                        <td height="25" width="500" align="right">
                            <table height="20" id="tblMess" align="center" style="display:none;">
        ...
        <form enctype="multipart/form-data" name="form2" method="post" action="">
            <table width="624" height="63" border="1" cellpadding="0" cellspacing="0" style="text-align:center" s>
                <tr>
                    <td width="432" height="20" valign="middle" bgcolor="#CCCCCC"><strong>所屬大類: <font color="#666666"
                                                                                                      size="+2">
                                <?= $_REQUEST[name] ?>
                            </font></strong></td>
                ...
                while ($rec = mysqli_fetch_array($res)) {
                    ...
                    <tr>
                        ...
                        <td width="49"><input type="button" id="but_update<?= $rec[int_id] ?>" value="修改"
                                              onClick="update_lib(<?= $rec[int_id] ?>)">
                            <input type="button" id="but_save<?= $rec[int_id] ?>" style=" display:none" value="保存"
                                   onClick="save_lib(<?= $rec[int_id] ?>,2,<?= $_REQUEST[int_main] ?>)"></td>
                        <td width="48">
                            <input type="button" id="but_del<?= $rec[int_id] ?>" value="刪除"
                                   onClick="delete_lib(<?= $rec[int_id] ?>,2,<?= $_REQUEST[int_main] ?>)">
                            <input type="button" id="but_res<?= $rec[int_id] ?>" style=" display:none" value="取消"
                                   onClick="res_lib(<?= $rec[int_id] ?>)"></td>
                    </tr>
                    <?php
                }
                ?>
