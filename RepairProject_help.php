...
if(isset($_POST['submit'])) {
	...
	if($submit == "更新"){
		...
		if($update_Result > 0) {
			?><script language="javascript">alert('紀錄更新成功!'); top.location.href = "CMS_RepairProject_manage.php"</script><?php 
		}
	}
	else if( $submit == "item"){
	    $pid = $_POST['pid'];
...
<form name="form_CaseUpdate" id="form" action="repairproject_help.php?type=<?=$_GET[type]?>&id=<?=$_GET["id"]?>" method="post" onSubmit="return sendAddSubmit();">
<table width="100%" border="1" style="border-collapse:collapse;" borderColor="black"cellspacing="0" cellpadding="0">
	...
	<tr bgcolor="CCFFFF" id="genus_tr">
        ...
        <td align="left" >
		<label for="type_t_1"><input  name="type_t" id="type_t_1" type="radio"   value="0"  <?php echo $record_completed[int_type]==0?"checked='checked'":""?> >分店</label>
		<label for="type_t_2"><input  name="type_t" id="type_t_2" type="radio"  value="2"  <?php echo $record_completed[int_type]==2?"checked='checked'":""?>>後勤</label>
		</td>
	</tr>
	<tr bgcolor="CCFFFF">
