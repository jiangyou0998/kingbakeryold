...
function popupPanel(item){
	...
	chr_sap=item.getAttribute("chr_sap");
	
	var _mediaHTML= "";
	
	if(item_media_url != null && item_media_url.length > 0){
		if(item_media_url.indexOf('.swf') == -1  ){
			_mediaHTML="<img height=51px'   src='"+item_media_url+"' style='border: 0px solid #999999; margin:0px 0px 0px 0px;'/>";
			//_mediaHTML="<a href='"+item_media_url+"' class='jqzoom' ><img height=51px'   src='"+item_media_url+"' style='border: 0px solid #999999; margin:0px 0px 0px 0px;'/></a>";
		}else{
			_mediaHTML += '<object ';
	...
	result = result.replace('p{popup_name}',item_name+"<br>");
	...
	result = result.replace('p{txt_datil_1}',txt_datil_1+"<br>");
	result = result.replace('p{txt_datil_2}',txt_datil_2+"<br>");
	result = result.replace('p{txt_datil_3}',txt_datil_3+"<br>");
	result = result.replace('p{chr_no}',chr_no+"<br>");
	result = result.replace('p{chr_sap}',chr_sap+"<br>");
...
function popupPanel2(item){
	...
	result = result.replace('p{chr_sap}',chr_sap);
	return result;
}

function popupPanelWH(item){
	var result = "";
	item_media_url=item.getAttribute("item_media_url");
	item_name=item.getAttribute("item_name");
	item_sap=item.getAttribute("item_sap");
	item_brand=item.getAttribute("item_brand");
	item_spec=item.getAttribute("item_spec");
	item_color=item.getAttribute("item_color");
	txt_datil_1=item.getAttribute("txt_datil_1");
	...
	var result = document.getElementById('popupPanel').innerHTML;
	result = result.replace('p{popup_name}',item_name+"<br>");
	result = result.replace('p{popup_sap}',item_sap);
	result = result.replace('p{popup_imageUrl}',_mediaHTML);
	result = result.replace('p{popup_brand}',item_brand);
	result = result.replace('p{popup_spec}',item_spec);
	result = result.replace('p{popup_color}',item_color);
...
function popupPanelWH2(item){
	var result = "";
	cut_time = item.getAttribute("cut_time");
	
	var result = document.getElementById('popupPanel').innerHTML;
	result = result.replace('p{cut_time}',cut_time);
